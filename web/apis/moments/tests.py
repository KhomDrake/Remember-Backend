...
class MemoryLineTest(APITestCase):
    ...
    def detailMoments(self, momentsId):
        return self.client.get(
            "/api/v1/moments/" + momentsId + "/",
            format='json', 
            HTTP_AUTHORIZATION=self.bearerToken
        )
    
    def deleteMoments(self, momentsId):
    ...
    def setUp(self):
        ...
        self.memoryLines = json.loads(self.memoryLineByType(typeId).content)

    def test_create_moment(self):
        memoryLineId = self.memoryLines["results"][0]["id"]
        self.createMoment(memoryLineId, "Testando")

        moment = Moment.objects.all().first()

        self.assertEqual(str(moment.memory_line.id), memoryLineId)
        self.assertEqual(moment.description, "Testando")

    def test_get_moments(self):
        memoryLineId = self.memoryLines["results"][0]["id"]
        self.createMoment(memoryLineId, "Testando 1")
        self.createMoment(memoryLineId, "Testando 2")
        self.createMoment(memoryLineId, "Testando 3")

        moments = json.loads(self.getMoments(memoryLineId).content)

        self.assertEqual(moments["count"], 3)
        self.assertEqual(moments["previous"], None)
        self.assertEqual(moments["next"], None)

        self.assertEqual(moments["results"][0]["description"], "Testando 3")
        self.assertEqual(moments["results"][1]["description"], "Testando 2")
        self.assertEqual(moments["results"][2]["description"], "Testando 1")

    def test_get_moments_has_second_page(self):
        memoryLineId = self.memoryLines["results"][0]["id"]
        self.createMoment(memoryLineId, "Testando 1")
        self.createMoment(memoryLineId, "Testando 2")
        self.createMoment(memoryLineId, "Testando 3")
        self.createMoment(memoryLineId, "Testando 4")
        self.createMoment(memoryLineId, "Testando 5")
        self.createMoment(memoryLineId, "Testando 6")
        self.createMoment(memoryLineId, "Testando 7")

        moments = json.loads(self.getMoments(memoryLineId).content)

        self.assertEqual(moments["count"], 7)
        self.assertEqual(moments["previous"], None)
        self.assertTrue(moments["next"] != None)

        self.assertEqual(moments["results"][0]["description"], "Testando 7")
        self.assertEqual(moments["results"][1]["description"], "Testando 6")
        self.assertEqual(moments["results"][2]["description"], "Testando 5")
        self.assertEqual(moments["results"][3]["description"], "Testando 4")
        self.assertEqual(moments["results"][4]["description"], "Testando 3")
        self.assertEqual(moments["results"][5]["description"], "Testando 2")

    def test_get_moments_second_page(self):
        memoryLineId = self.memoryLines["results"][0]["id"]
        self.createMoment(memoryLineId, "Testando 1")
        self.createMoment(memoryLineId, "Testando 2")
        self.createMoment(memoryLineId, "Testando 3")
        ...
        moments = json.loads(self.getMoments(memoryLineId, page=2).content)

        self.assertEqual(moments["count"], 7)
        self.assertEqual(moments["next"], None)
        self.assertTrue(moments["previous"] != None)

        self.assertEqual(moments["results"][0]["description"], "Testando 1")

    def test_get_moments_second_page_less_than_six_moments(self):
        memoryLineId = self.memoryLines["results"][0]["id"]
        self.createMoment(memoryLineId, "Testando 1")
        self.createMoment(memoryLineId, "Testando 2")
        self.createMoment(memoryLineId, "Testando 3")

        response = self.getMoments(memoryLineId, page=2)
        self.assertEqual(response.status_code, 404)

    def test_detail_moments(self):
        memoryLineId = self.memoryLines["results"][0]["id"]
        self.createMoment(memoryLineId, "Testando 1")

        moment = json.loads(self.getMoments(memoryLineId).content)["results"][0]
        momentDetail = json.loads(self.detailMoments(moment["id"]).content)

        owner = RememberAccount.objects.first().id

        self.assertEqual(momentDetail["description"], "Testando 1")
        self.assertEqual(momentDetail["comments_count"], 0)
        self.assertEqual(momentDetail["memory_line"], memoryLineId)
        self.assertEqual(momentDetail["owner"]["id"], str(owner))

    def test_delete_moments(self):
        memoryLineId = self.memoryLines["results"][0]["id"]
        self.createMoment(memoryLineId, "Testando 1")
        self.createMoment(memoryLineId, "Testando 2")
        ...
        self.deleteMoments(moment["id"])

        momentNotDeleted = json.loads(self.getMoments(memoryLineId).content)["results"][0]
        self.assertEqual(momentNotDeleted["description"], "Testando 1")

    def test_get_moments_another_account_should_not_work(self):
        memoryLineId = self.memoryLines["results"][0]["id"]
        self.createMoment(memoryLineId, "Testando 1")
        ...
        request = self.getMoments(memoryLineId)
        self.assertEqual(request.status_code, 403)

    def test_detail_moment_another_account_should_not_work(self):
        memoryLineId = self.memoryLines["results"][0]["id"]
        self.createMoment(memoryLineId, "Testando 1")
        ...
        request = self.detailMoments(moment["id"])
        self.assertEqual(request.status_code, 403)

    def test_delete_moment_another_account_should_not_work(self):
        memoryLineId = self.memoryLines["results"][0]["id"]
        self.createMoment(memoryLineId, "Testando 1")
        ...
        request = self.deleteMoments(moment["id"])
        self.assertEqual(request.status_code, 403)

    def test_create_moments_and_get_memory_line_should_show_last_two(self):
        memoryLineId = self.memoryLines["results"][0]["id"]
        self.createMoment(memoryLineId, "Testando 1")
        self.createMoment(memoryLineId, "Testando 2")
        self.createMoment(memoryLineId, "Testando 2")
        self.createMoment(memoryLineId, "Testando 2")
        self.createMoment(memoryLineId, "Testando 2")
        moment1 = json.loads(self.createMoment(memoryLineId, "Testando 3").content)
        moment2 = json.loads(self.createMoment(memoryLineId, "Testando 4").content)
        request = json.loads(self.memoryLineByType(self.typeId).content)
        moments = request["results"][0]["some_moments"]
        self.assertEqual(moments[0]["id"], moment2["id"])
        self.assertEqual(moments[1]["id"], moment1["id"])